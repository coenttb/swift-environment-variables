...
extension EnvironmentVariables {
    ...
    @available(
        *,
        deprecated,
        message: "Use live(environmentConfiguration:requiredKeys:decoder:) instead."
    )
    ...
    public static func live(
        ...
        do {
            ...
            let localEnvVarDict = try loadEnvironmentFiles(
    ...
    private static func getLocalEnvironment(
        ...
        do {
            ...
            logger.warning(
                "Could not load local environment from \(url.path): \(error.localizedDescription)"
            )
            return [:]
        }
    }
    ...
    private static func parseValue(_ value: String) -> String {
        ...
        if !workingValue.hasPrefix("\"") && !workingValue.hasPrefix("'") {
            if let commentIndex = workingValue.firstIndex(of: "#") {
                ...
                workingValue = String(workingValue[..<commentIndex]).trimmingCharacters(
                    in: .whitespaces
                )
            }
        }
